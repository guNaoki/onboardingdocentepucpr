      ...
      <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {guidesData.map((guide) => (
          <StaggerItem key={guide.id}>
            <Link 
              href={`/guias/${guide.id}`}
              className="group bg-white border border-border-color rounded-radius p-8 flex flex-col transition-all duration-300 hover:-translate-y-1 hover:shadow-hover hover:border-puc-red h-full"
            >
